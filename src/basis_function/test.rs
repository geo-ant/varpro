...
fn callable_evaluation_using_the_generic_interface() {
    ...
    assert_eq!(eval_callable(|x:&DVector<f64>,a,b,c|{a*b*c*x.clone()},&x, &[2.,3.,4.]),2.*3.*4.*x.clone());
    assert_eq!(eval_callable(|x:&DVector<f64>,a,b,c,d|{a*b*c*d*x.clone()},&x,&[2.,3.,4.,5.]),2.*3.*4.*5.*x.clone());
    assert_eq!(eval_callable(|x:&DVector<f64>,a,b,c,d,e|{a*b*c*d*e*x.clone()},&x,&[2.,3.,4.,5.,6.]),2.*3.*4.*5.*6.*x.clone());
    assert_eq!(eval_callable(|x:&DVector<f64>,a,b,c,d,e,f|{a*b*c*d*e*f*x.clone()},&x,&[2.,3.,4.,5.,6.,7.]),2.*3.*4.*5.*6.*7.*x.clone());
    assert_eq!(eval_callable(|x:&DVector<f64>,a,b,c,d,e,f,g|{a*b*c*d*e*f*g*x.clone()},&x,&[2.,3.,4.,5.,6.,7.,8.]),2.*3.*4.*5.*6.*7.*8.*x.clone());
...
fn callable_argument_length_is_correct() {
    ...
    assert_eq!((|x:&DVector<f64>,a,b,c|{x.clone()}).argument_count(),3);
    assert_eq!((|x:&DVector<f64>,a,b,c,d|{x.clone()}).argument_count(),4);
    assert_eq!((|x:&DVector<f32>,a,b,c,d,e|{x.clone()}).argument_count(),5);
    assert_eq!((|x:&DVector<f32>,a,b,c,d,e,f|{x.clone()}).argument_count(),6);
    assert_eq!((|x:&DVector<f32>,a,b,c,d,e,f,g|{x.clone()}).argument_count(),7);
