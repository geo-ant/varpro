...
fn diagonal_matrix_matrix_and_matrix_vector_product_produces_correct_results() {
    ...
    assert_eq!(&D1*&A,&D2*&A, "Diagonal matrix-matrix multiplication must produce correct result.");
    assert_eq!(&D1*&v,&D2*&v, "Diagonal matrix-vector multiplication must produce correct result.");
