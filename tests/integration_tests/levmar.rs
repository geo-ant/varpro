...
use nalgebra::{Dynamic, Scalar, Matrix, Vector, DVector, DMatrix, RowDVector, U5, Vector5};
...
impl LeastSquaresProblem<f64,Dynamic,U5> for DoubleExponentialDecayFittingWithOffset {
    ...
    fn residuals(&self) -> Option<DVector<f64>> {
        ...
        let f = c1*&self.precalc_exp_tau1 +c2*&self.precalc_exp_tau2 + &DVector::from_element(self.x.len(),c3);
    ...
    fn jacobian(&self) -> Option<Matrix<f64, Dynamic, U5, Self::JacobianStorage>> {
        ...
        let mut jacobian  = unsafe {Matrix::<f64, Dynamic, U5, Self::JacobianStorage>::new_uninitialized(nrows)};
